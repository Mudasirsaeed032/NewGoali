...
const LoginPage = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Step 2: Send credentials to the backend to retrieve the JWT token
      ...
      if (result.token) {
        ...
        const userRole = result.user?.role || data.user?.user_metadata?.role || "dashboard"
                ...
                <motion.button
                  ...
                  className={`w-full py-4 rounded-xl font-header text-lg transition-all duration-200 ${
                    isLoading || !email || !password
                      ...
                      : `bg-gradient-to-r from-blue-500 to-blue-600 hover:shadow-lg hover:scale-[1.02]`
                  } text-white`}
